...
| # | Steps | Expected |
|---|---|---|
| 1 | Open http://localhost:4173 in Chrome, DevTools → Application → Manifest | Name "Form Forge for ODK", theme `#3e9fcc`, standalone, 4 icons incl. maskable; no installability warnings |
...
| 3 | Chrome address bar → Install icon ("Install Form Forge for ODK") | App installs and opens in its own standalone window with the FB icon |
...
| 6 | Still with the app open in the library, create+save a form, then check the library footer | Footer shows `Form Forge for ODK v<version>` and either "Storage: persistent" (once granted) or the dismissible "export a workspace backup" hint whose link downloads an archive; the dismiss X hides it across reloads |
...
| 9 | `BASE_PATH=/form-forge/ pnpm build` and serve `dist/` under `/form-forge/` | App, manifest (`start_url: '.'`) and SW all work under the prefix |
