...
// Contract suite: workspace-io must behave identically on the Dexie default
// and on embed mode's memory backend (tests/helpers/backends.ts).
describe.each(backendCases)('workspace-io ($name backend)', ({ setup }) => {
  beforeEach(setup)
  ...
  describe('importArchiveForms', () => {
    ...
    it('isolates per-form failures: a failing write leaves earlier imports committed', async () => {
      ...
      const realImport = backend.importForm.bind(backend)
      let calls = 0
      const spy = vi.spyOn(backend, 'importForm').mockImplementation(async (record, attachments) => {
        if (++calls === 2) throw new Error('write failed')
        return realImport(record, attachments)
      })
      try {
    ...
    it('round-trips gather → build → read → import losslessly', async () => {
      ...
      expect(await formsRepo.listForms()).toEqual([])
